...
def get_columns(filters):
    ...
    if filters.get("show_completed_tasks"):
        columns.insert(9, {  # Adjust index due to new columns
            "fieldname": "completed_on",
            "label": _("Completed On"),
            "fieldtype": "Date",
            "width": 120
...
def prepare_data(filters, projects, tasks):
    ...
    if assignees:
        ...
        user_first_names = {u.name: u.first_name for u in users}

    # Build parent-children mapping
    for t in tasks:
        if t.parent_task:
            parent_children_map.setdefault(t.parent_task, []).append(t)
        else:
            parent_children_map.setdefault(None, []).append(t)

    # Group strictly employee-wise
    for assignee in assignees:
        employee_tasks = [t for t in tasks if t.assignee == assignee]

        if not employee_tasks:
            continue

        total_expected = sum(
            float(t.expected_time or 0)
            for t in employee_tasks
            if t.status != "Open"
        )
        total_actual = sum(
            float(t.actual_time or 0)
            for t in employee_tasks
            if t.status != "Open"
        )
        ...
        # --- Add summary row ---
        data.append({
            "task": f"{user_first_names.get(assignee, assignee)}",
            "expected_time": round(total_expected, 2),
            ...
            # Add only this employee's tasks
        for task in employee_tasks:
            if not task.parent_task:
                add_task_to_data(
                    data, task, parent_children_map, 1, user_first_names, assignee_filter=assignee
                )
...
def add_task_to_data(data, task, parent_children_map, level, user_first_names, show_progress=False, assignee_filter=None):
    # Skip tasks of other employees (important!)
    ...
    employee_name = frappe.db.get_value("User", task.assignee, 'full_name')
    ...
    data.append(frappe._dict({
        ...
        "actual_time": f"{float(task.actual_time or 0):.1f}",
    ...
    # Process only same-employee children
    children = parent_children_map.get(task.name, [])
    for child in children:
        add_task_to_data(data, child, parent_children_map, level + 1, user_first_names, show_progress, assignee_filter)
