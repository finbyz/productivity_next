...
def get_heatmap_data(user, date):
    ...
    for item in data:
        ...
        if date not in combined_data:
            combined_data[date] = {"count": 0, "fincall_count": 0, "activity_count": 0}
...
def get_user_data(user, start_date=None, end_date=None):
    ...
    fincall_time_data = frappe.db.sql(
        f"""
    ...
    meeting_time_data = frappe.db.sql(
        f"""
    ...
    incoming_fincall_count = outgoing_fincall_count = missed_fincall_count = (
        rejected_fincall_count
    ) = 0
    ...
    internal_fincall_data = frappe.db.sql(
        f"""
        SELECT 
            calltype,
            COUNT(*) AS fincall_count,
    ...
    list_data = []
    meeting_total_data = frappe.db.sql(
        f"""
    ...
    calls_total_data = frappe.db.sql(
        f"""
        SELECT call_datetime as start_time, ADDTIME(call_datetime, SEC_TO_TIME(duration)) as end_time
        FROM `tabEmployee Fincall`
    ...
    application_total_data = frappe.db.sql(
        f"""
    ...
    application_usage_days = frappe.db.sql(
        f"""
    ...
    meeting_usage_days = frappe.db.sql(
        f"""
    ...
    # Documents Accessed
    total_unique_doc = frappe.db.sql(
        f"""
    ...
    # TABLES BELOW CARDS
    application_name = frappe.db.sql(
        f"""
    ...
    return {
        ...
        "total_unique_doc": total_unique_doc[0]["activity_count"]
        if total_unique_doc
        else 0,
        "total_time_on_calls": fincall_data[0]["total_duration"] if fincall_data else 0,
        "total_meeting_duration_internal": meetings_internal_employee[
            0
        ].total_meeting_duration
        if meetings_internal_employee
        else 0,
        "total_meeting_count_internal": meetings_internal_employee[0].meeting_count
        if meetings_internal_employee
        else 0,
        "total_meeting_duration_external": meetings_external_employee[
            0
        ].total_meeting_duration
        if meetings_external_employee
        else 0,
        "total_meeting_count_external": meetings_external_employee[0].meeting_count
        if meetings_external_employee
        else 0,
...
def get_linechart_data(user, start_date=None, end_date=None):
    ...
    aggregated_list = [
        {
            "hour": hour,
            "incoming_count": counts["incoming_count"],
            "outgoing_count": counts["outgoing_count"],
        }
    ...
    return {
        ...
        "datasets": [
            {
                "name": "Incoming",
                "values": [i["incoming_count"] for i in aggregated_list],
            },
            {
                "name": "Outgoing",
                "values": [i["outgoing_count"] for i in aggregated_list],
            },
        ],
    }
...
def get_activity_chart_data():
    ...
    # TODO : return data in %
    sample_data = {"incoming": 50, "outgoing": 20, "missed": 20, "rejected": 10}
    return sample_data
